...
const ScrollPathSVG = () => {
  ...
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start end", "end start"] // L'animazione parte quando il top del container entra, e finisce quando il bottom esce
  });
  ...
  return (
    // Questo div ref definisce l'area in cui lo scroll è tracciato
    <div ref={ref} className="absolute inset-0 z-10 pointer-events-none">
      ...
      >
        ...
        <motion.path
          d="M 50 0 
             C 50 300, 20 500, 50 700 
             L 50 1000"
          fill="none"
          stroke="#FACC15" // Giallo Elettrico (yellow-400)
          strokeWidth="4"
          strokeLinecap="round"
          opacity="0.3"
...
=> (
            <a
key={link.name} href={link.href} className="text-sm font-medium
text-slate-300 hover:text-yellow-400 transition-colors relative group">
              {link.name}
              <span
...
text="Preventivo Gratuito" />
        ...
        {/* Toggle Mobile */}
        <button
className="md:hidden text-white p-2 focus:outline-none" onClick={()
=> setIsOpen(!isOpen)} aria-label="Toggle Menu">
          {isOpen ? <X
className="w-6 h-6" /> : <Menu className="w-6 h-6"
/>}
        </button>
      </div>
...
=> (
            <a
key={link.name} href={link.href} className="text-lg font-medium text-white
hover:text-yellow-400 border-b border-slate-800/50 pb-2" onClick={() =>
setIsOpen(false)}>
              {link.name}
            </a>
          ))}
          <ElectricButton
text="Preventivo Gratuito" onClick={() => setIsOpen(false)} />
        </div>
      </motion.div>
    </nav>
  );
...
variants={textVariants} className="flex flex-wrap gap-3 mb-6">
            <ServiceTag
text="PRONTO INTERVENTO H24" icon={Clock} />
            <ServiceTag
text="CERTIFICAZIONE GARANTITA" icon={CheckCircle2} />
          </motion.div>
...
leading-relaxed"
          >
            Specialisti in
progettazione, installazione e manutenzione di impianti elettrici certificati,
domotici e fotovoltaici.
          </motion.p>
          
          <motion.div
variants={textVariants} className="flex flex-wrap gap-4
items-center">
            <ElectricButton
text="Richiedi Sopralluogo" />
            <a
href="#servizi" className="px-6 py-3 group flex items-center
gap-2 font-semibold text-white border-2 border-transparent
hover:border-yellow-400/50 rounded-lg transition-colors">
              
Scopri di più
              
<ChevronRight className="w-5 h-5 text-yellow-400 group-hover:text-white
group-hover:translate-x-1 transition-transform" />
            </a>
          </motion.div>
        </motion.div>
        
        {/* Icona 3D Impianto (Elemento
visivo) */}
        <div
...
bg-slate-900 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.5)] border
border-white/10"
          >
            <BatteryCharging className="w-40 h-40 text-yellow-400 opacity-20
relative z-10" />
            <Zap className="w-20 h-20 text-yellow-400 absolute
drop-shadow-[0_0_20px_rgba(250,204,21,0.8)] z-20" />
            {/* Dettagli in background */}
            <div className="absolute -top-4 -right-4 w-12 h-12 bg-blue-600/30
rounded-full blur-xl" />
            <div className="absolute bottom-4 left-4 w-10 h-10
bg-yellow-400/30 rounded-full blur-xl" />
          </motion.div>
        </div>
      </div>
...
font-bold text-white mb-3">{title}</h3>
      <p
...
const ServicesGrid = () => {
    const services = [
      { icon: Home, title: "Impianti
Residenziali", desc: "Sistemi domotici intelligenti, gestione carichi
e quadri elettrici a norma per la massima sicurezza e comfort in casa." },
      { icon: Factory, title: "Settore
Industriale", desc: "Cabine di trasformazione, power center,
automazione industriale e manutenzione predittiva per la continuità
operativa." },
      { icon: ShieldCheck, title:
"Sicurezza & TVCC", desc: "Sistemi antintrusione connessi,
videosorveglianza IP ad alta risoluzione e controlli accessi perimetrali."
},
      { icon: BatteryCharging, title:
"Fotovoltaico & Storage", desc: "Progettazione e
installazione di impianti fotovoltaici con accumulatori per l'indipendenza
energetica totale." },
      { icon: Lightbulb, title:
"Illuminazione LED", desc: "Soluzioni di illuminazione ad alta
efficienza e design, incluse l'illuminazione pubblica e industriale." },
      { icon: Cpu, title: "Automazione
Domotica", desc: "Integrazione completa di riscaldamento, clima,
sicurezza e multimedia in un unico sistema gestibile da remoto." },
    ];
 
    return (
      <section
id="servizi" className="py-24 bg-slate-950">
          <div
...
mb-2">I Nostri Servizi Core</p>
                  
<h2 className="text-4xl md:text-5xl font-bold
text-white">Tecnologia, Sicurezza, Efficienza.</h2>
</div>
...
font-bold text-white mb-2">{title}</h3>
      <p
className="text-slate-400">{desc}</p>
    </div>
  </motion.div>
...
const Benefits = () => {
  const benefits = [
    { icon: ClipboardCheck, title:
"Certificazione Garantita", desc: "Tutti i nostri impianti sono
rilasciati con dichiarazione di conformità (DiCo) secondo le normative CEI in
vigore." },
    { icon: TrendingUp, title: "Efficienza
Massima", desc: "Utilizziamo solo componenti ad alta efficienza per
minimizzare i consumi e massimizzare il risparmio energetico." },
    { icon: Layers, title: "Soluzioni Chiavi
in Mano", desc: "Dalla progettazione alla messa in opera, gestiamo
ogni fase del tuo progetto senza stress per te." },
    { icon: HardHat, title: "Team
Qualificato", desc: "I nostri tecnici sono costantemente aggiornati
sulle ultime tecnologie e protocolli di sicurezza." },
  ];
...
className="lg:pr-10">
            <p
className="text-yellow-400 uppercase font-bold text-sm mb-2">Il
Nostro Valore Aggiunto</p>
            <h2
className="text-4xl md:text-5xl font-bold text-white
mb-6">Scegliere Flash Impianti è Scegliere la Qualità.</h2>
            <p
className="text-slate-300 text-lg mb-8">
              Non siamo
solo installatori, siamo consulenti energetici. Ti aiutiamo a fare scelte
informate che impattano positivamente sull'ambiente e sul tuo bilancio.
            </p>
            <ElectricButton
...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // In un ambiente reale, qui ci sarebbe la
logica per inviare i dati.
    console.log("Dati inviati per il
preventivo.");
    // Simulo un messaggio di successo
    
document.getElementById('contact-message').textContent = "Richiesta
inviata! Sarai ricontattato a breve.";
...
className="max-w-4xl mx-auto text-center mb-12">
            <h2
className="text-4xl md:text-5xl font-bold text-white
mb-6">Accendiamo il tuo Progetto?</h2>
            <p
className="text-slate-400 text-lg">Inviaci i dettagli per un
preventivo gratuito e non vincolante.</p>
          </div>
...
duration: 0.7 }}
          ...
          >
            <div
id="contact-message" className="hidden p-3 bg-yellow-400
text-slate-950 font-semibold rounded-lg text-center mb-4
transition-all"></div>
            <input
type="text" placeholder="Nome Completo" required
className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4
py-3 text-white placeholder-slate-500 focus:border-yellow-400 focus:ring-1
focus:ring-yellow-400 focus:outline-none transition-colors" />
            <input
...
required className="w-full bg-slate-950 border border-slate-700 rounded-lg
px-4 py-3 text-white placeholder-slate-500 focus:border-yellow-400 focus:ring-1
focus:ring-yellow-400 focus:outline-none transition-colors" />
            <textarea
rows="4" placeholder="Descrivi la tua esigenza (tipo di
impianto, ubicazione, urgenza...)" required className="w-full
bg-slate-950 border border-slate-700 rounded-lg px-4 py-3 text-white
placeholder-slate-500 focus:border-yellow-400 focus:ring-1
focus:ring-yellow-400 focus:outline-none transition-colors
resize-none"></textarea>
            
            <button
type="submit" className="w-full py-3 bg-yellow-400
text-slate-900 font-bold rounded-lg hover:bg-yellow-300 transition-all
uppercase text-lg shadow-xl shadow-yellow-400/30">
              Invia la
Mia Richiesta
            </button>
          </motion.form>
...
gap-2 items-center md:items-start">
        <Logo />
        <p className="text-xs
mt-2 text-slate-600">P.IVA 01234567890 | Via dell'Energia, 1 - 00100
Roma (RM)</p>
        <p className="text-xs
text-slate-400">© 2024 FlashImpianti. Tutti i diritti
riservati.</p>
      </div>
      <div className="flex gap-4
text-sm font-medium">
        <a href="#"
className="hover:text-yellow-400 transition-colors">Privacy
Policy</a>
        <a href="#"
className="hover:text-yellow-400 transition-colors">Termini di
Servizio</a>
      </div>
    </div>
  </footer>
